document.addEventListener("DOMContentLoaded", function () {
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", function (e) {
      ...
      // PERINGATAN KEAMANAN: Membandingkan password teks biasa sangat tidak aman.
      ...
      if (userDitemukan) {
        ...
        localStorage.setItem("namaUserAktif", userDitemukan.name);
...
function prosesLoginSosial(email, provider) {
  // Ambil bagian sebelum "@" sebagai nama sementara
  const name = email.split("@")[0];
  localStorage.setItem("isLoggedIn", "true");
  localStorage.setItem("namaUserAktif", name);
...
function logout() {
  if (confirm("Apakah kamu yakin ingin keluar?")) {
    ...
    localStorage.removeItem("pesananTerakhir");
    window.location.href = "index.html";
  }
}
